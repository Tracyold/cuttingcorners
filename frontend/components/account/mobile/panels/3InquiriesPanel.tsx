...
function InquiryBody({ inq, archived = false }: { inq: InquiryPanelItem; archived?: boolean }) {
  ...
  if (inq.products?.total_price != null) productMetaParts.push(`$${Number(inq.products.total_price).toLocaleString()}`);
  ...
  return (
    <>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
        ...
        <span style={{
          fontFamily: 'var(--font-mono-mob)', fontSize: 'clamp(9px,2.4vw,10px)',
          letterSpacing: '0.15em', textTransform: 'uppercase',
          padding: '3px 10px', flexShrink: 0, borderRadius: 999,
          background: archived ? 'rgba(127,127,127,0.1)' : (isPending ? 'rgba(207,221,78,0.1)' : 'rgba(45,212,191,0.1)'),
          color: archived ? 'var(--text-mob-muted)' : (isPending ? 'var(--gold)' : '#2dd4bf'),
        }}>
          {archived ? 'Archived' : (isPending ? 'Pending' : 'Replied')}
        </span>
      </div>

      <div style={{ fontFamily: 'var(--font-ui-mob)', fontSize: 'clamp(13px,3.4vw,14px)', color: 'var(--text-mob-muted)', lineHeight: 1.6, borderTop: '0.5px solid var(--bdr2-mob)', paddingTop: 8, marginTop: 4 }}>
        "{inq.description}"
      </div>
      ...
      {inq.reply && (
        <div style={{ fontFamily: 'var(--font-ui-mob)', fontSize: 'clamp(13px,3.4vw,14px)', color: 'var(--text-mob)', lineHeight: 1.6, borderTop: '0.5px solid var(--bdr2-mob)', paddingTop: 8, marginTop: 8 }}>
          <span style={{ fontFamily: 'var(--font-mono-mob)', fontSize: 'clamp(9px,2.4vw,10px)', color: 'var(--gold)', letterSpacing: '0.1em', textTransform: 'uppercase', display: 'block', marginBottom: 4 }}>
            CCG replied
          </span>
          "{inq.reply}"
          {inq.replied_at && (
...
function SwipeableInquiry({ inq, onArchive, onTap }: { inq: InquiryPanelItem; onArchive: (id: string) => void; onTap: () => void }) {
  ...
  return (
    <div className="sr-card-wrap">
      <button className="sr-archive-btn" onClick={e => { e.stopPropagation(); onArchive(inq.account_inquiry_id); }}>
        Archive
      </button>
      <div
  ...
  return (
    <div className="sr-card-wrap" onClick={onTap} style={{ cursor: 'pointer' }}>
      <div style={{
        minHeight: 'auto',
        padding: 'clamp(1rem,4.5vw,1.25rem) clamp(1.125rem,5vw,1.5rem)',
        borderRadius: 12, opacity: 0.65, filter: 'grayscale(0.4)',
  ...
  return (
    <>
      <div ref={elementRef} className={`slide-panel${open ? ' open' : ''}`}>
        ...
        {archiveError && (
          <div style={{ background: 'var(--bg-card)', color: 'var(--text-mob-muted)', padding: 'clamp(0.75rem,3.5vw,1rem)', margin: 'clamp(0.5rem,2.5vw,0.75rem)', borderRadius: 8, fontSize: 'clamp(13px,3.4vw,14px)', lineHeight: 1.5, textAlign: 'center' }}>
            {archiveError}
          </div>
        )}
        ...
        <div className="sr-list">
          ...
          {activeTab === 'active' && activeList.length > 0 && (
            <p style={{ fontFamily: 'var(--font-ui-mob)', fontSize: 'clamp(13px,3.4vw,14px)', color: 'var(--text-mob-muted)', textAlign: 'center', padding: '8px 0', opacity: 0.6, lineHeight: 1.6 }}>
