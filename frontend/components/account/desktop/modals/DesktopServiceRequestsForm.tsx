// components/account/desktop/DesktopServiceRequestForm.tsx
//
// Desktop version of ServiceRequestForm.tsx.
// Identical except nr-overlay and nr-sheet are scoped to the right 50%
...
export default function DesktopServiceRequestForm({
  ...
  const addCustomField = () => {
    ...
    setCustomFields(prev => [...prev, { id: uid(), label: `Custom field ${prev.length + 1}`, value: '', editingLabel: true }]);
  };
  ...
  const handleFilesPicked = async (files: FileList | null) => {
    ...
    for (const file of toProcess) {
      ...
      setPhotos(prev => [...prev, { tempId, fileName: file.name, objectUrl, storagePath, publicUrl: null, uploading: true, uploaded: false, showSuccessPill: false, error: null }]);
      (async () => {
        try {
          const result = await Promise.race([
            supabase.storage.from('service-request-photos').upload(storagePath, file, { cacheControl: '3600', upsert: false }),
            new Promise<{ data: null; error: { message: string } }>((resolve) => setTimeout(() => resolve({ data: null, error: { message: 'Upload timed out.' } }), 60_000)),
          ]);
          const upErr = (result as any)?.error;
          if (upErr) { setPhotos(prev => prev.map(p => p.tempId === tempId ? { ...p, uploading: false, error: upErr.message } : p)); return; }
          let publicUrl: string | null = null;
          try { const { data: pub } = supabase.storage.from('service-request-photos').getPublicUrl(storagePath); publicUrl = pub?.publicUrl ?? null; } catch {}
          setPhotos(prev => prev.map(p => p.tempId === tempId ? { ...p, uploading: false, uploaded: true, publicUrl, showSuccessPill: true, error: null } : p));
          setTimeout(() => setPhotos(prev => prev.map(p => p.tempId === tempId ? { ...p, showSuccessPill: false } : p)), 2000);
        } catch (err: any) {
          setPhotos(prev => prev.map(p => p.tempId === tempId ? { ...p, uploading: false, error: err?.message ?? 'Upload failed' } : p));
        }
      })();
    }
  ...
  const removePhoto = async (tempId: string) => {
    ...
    if (target.uploaded && target.storagePath) {
      try { await supabase.storage.from('service-request-photos').remove([target.storagePath]); } catch {}
    }
  };

  const anyPhotoUploading = photos.some(p => p.uploading);
  const contactComplete = finalName.length > 0 && finalEmail.length > 0 && finalPhone.length > 0 && finalAddress.length > 0;
  const consentSatisfied = existingConsent.consented ? true : consentChecked;

  const handleSubmit = async () => {
    setSubmitError(null);
    if (!session?.user?.id) { setSubmitError('Session missing. Please log out and log back in.'); return; }
    if (submitting) return;
    if (anyPhotoUploading) { setSubmitError('Please wait for photos to finish uploading before submitting.'); return; }
    if (!contactComplete) {
    ...
    if (!srDesc.trim()) { setSubmitError('Please add a description.'); return; }
    if (!consentSatisfied) { setSubmitError('Please check the SMS notification consent box to submit.'); return; }
    setSubmitting(true);
    try {
      const qtyNum = parseInt(quantity, 10);
      const finalQty = Number.isFinite(qtyNum) && qtyNum >= 1 && qtyNum <= 99 ? qtyNum : 1;
      const now = new Date().toISOString();
      const consentAtToStore = existingConsent.consented ? (existingConsent.consentedAt ?? now) : (consentChecked ? now : null);
      const { data: inserted, error: insErr } = await supabase
        .from('service_requests')
        .insert({
          account_user_id: session.user.id, service_type: srType.trim() || null,
          description: srDesc.trim(), gem_type: gemType.trim() || null,
          gem_color: gemColor.trim() || null, shape: shape.trim() || null,
          ...
          contact_name: finalName, contact_email: finalEmail,
          contact_phone: finalPhone, contact_address: finalAddress,
        ...
        .maybeSingle();
      if (insErr || !inserted?.service_request_id) { setSubmitError(insErr?.message || 'Could not submit. Please try again.'); return; }
      const goodFields = customFields.filter(cf => cf.label.trim().length > 0).map((cf, i) => ({ service_request_id: inserted.service_request_id, account_user_id: session.user.id, label: cf.label.trim(), value: cf.value.trim() || null, sort_order: i }));
      if (goodFields.length > 0) await supabase.from('service_request_custom_fields').insert(goodFields);
      if (consentChecked && !existingConsent.consented) await supabase.from('user_sms_preferences').upsert({ user_id: session.user.id, opt_in_work_orders: true }, { onConflict: 'user_id' });
      try { await supabase.functions.invoke('send-admin-notification', { body: { event_type: 'service_requests', user_id: session.user.id } }); } catch {}
      await onSubmitted();
    } catch (err: any) {
  ...
  const InfoBtn = ({ id }: { id: string }) => (
    <button className={`nr-info-btn${activeTip === id ? ' active' : ''}`} onClick={() => toggleTip(id)} type="button">i</button>
  );
  const Tip = ({ id }: { id: string }) => (
    <div className={`nr-tooltip${activeTip === id ? ' show' : ''}`}>{TOOLTIPS[id]}</div>
  );
  const ReadContact = ({ label, value }: { label: string; value: string }) => (
    <div className="srf-contact-row"><span className="srf-contact-label">{label}</span><span className="srf-contact-value">{value}</span></div>
  );
  const EditContact = ({ label, value, onChange, type = 'text', placeholder }: { label: string; value: string; onChange: (v: string) => void; type?: string; placeholder: string }) => (
  ...
  return (
    <>
      ...
      {/* Sheet scoped to right 50% */}
      <div
        ref={elementRef}
        className={`nr-sheet${open ? ' open' : ''}`}
        style={{ left: '50%', right: 0, width: '50%' }}
      >
        ...
        <div className="nr-body">
          ...
          <div className="srf-section">
            ...
            <div className="srf-contact-block">
              {profileHasName    ? <ReadContact label="Name"     value={profile.name}             /> : <EditContact label="Name"     value={contactName}    onChange={setContactName}    placeholder="Your full name" />}
              {profileHasEmail   ? <ReadContact label="Email"    value={profile.email}            /> : <EditContact label="Email"    value={contactEmail}   onChange={setContactEmail}   placeholder="you@example.com" type="email" />}
              {profileHasPhone   ? <ReadContact label="Phone"    value={profile.phone}            /> : <EditContact label="Phone"    value={contactPhone}   onChange={setContactPhone}   placeholder="(555) 555-5555" type="tel" />}
              {profileHasAddress ? <ReadContact label="Shipping" value={profile.shipping_address} /> : <EditContact label="Shipping" value={contactAddress} onChange={setContactAddress} placeholder="Street, City, State, ZIP" />}
            </div>
          </div>
          ...
          <div className="srf-section">
            ...
            {customFields.map((cf) => (
              <div key={cf.id} className="srf-custom-row">
                {cf.editingLabel ? (
                  <input className="srf-custom-label-input" value={cf.label} autoFocus onChange={e => updateCustomField(cf.id, { label: e.target.value })} onBlur={() => updateCustomField(cf.id, { editingLabel: false, label: cf.label.trim() || 'Custom field' })} onKeyDown={(e) => { if (e.key === 'Enter') (e.target as HTMLInputElement).blur(); }} />
                ) : (
                  <button className="srf-custom-label-btn" onClick={() => updateCustomField(cf.id, { editingLabel: true })} type="button">{cf.label}</button>
                )}
          ...
          <div className="srf-section">
            <div className="nr-field">
              ...
              <Tip id="desc" />
              <textarea className="nr-input req" placeholder="Tell us about the stone and what you need..." value={srDesc} onChange={e => setSrDesc(e.target.value)} style={{ minHeight: '100px', fontFamily: 'var(--font-ui-mob)', resize: 'vertical' }} />
            </div>
          </div>
          ...
          <div className="srf-section">
            ...
            {photos.length < MAX_PHOTOS && (
              <div className="nr-photo-area" onClick={() => fileInputRef.current?.click()}>
                ...
                <div className="nr-photo-area-label">Tap to add photos<br /><span style={{ fontSize: 'clamp(10px, 2.6vw, 11px)', opacity: 0.6 }}>PNG · JPG · HEIC · WEBP · max 25MB · up to 3</span></div>
              </div>
            )}
            <input type="file" ref={fileInputRef} accept={ACCEPTED_MIME_HINT} multiple style={{ display: 'none' }} onChange={e => handleFilesPicked(e.target.files)} />
            {photos.length > 0 && (
            ...
            {existingConsent.consented ? (
              ...
              <div className="srf-consent-block">
                <label className="srf-consent-check">
                  ...
                  <span>I understand this service request may trigger a manual work order, and that I am requesting a service from Cutting Corners Gems. I understand that if a work order is created, I must opt in to text-message notifications on my cell phone for updates about the work order.</span>
                </label>
                <p className="srf-consent-note">If you have never had a work order in your account before, you will always be contacted before Cutting Corners opens a work order. Don't be worried about the checkbox — we need it to send you text-message notifications by law.</p>
              </div>
            )}
          </div>

          {submitError && <div ref={errorRef} className="srf-submit-error">{submitError}</div>}
        ...
        <div className="nr-footer">
          ...
          <div className="nr-req-note">Fields marked <span style={{ color: 'var(--gold)' }}>REQUIRED</span> must be filled before submitting.</div>
        </div>
      </div>

      <style jsx>{`
        :global(.srf-pair) { display: flex; gap: clamp(0.5rem, 2.5vw, 0.75rem); margin-bottom: clamp(0.75rem, 3.5vw, 1.125rem); flex-wrap: wrap; align-items: flex-start; }
        :global(.srf-pair-cell) { flex: 1 1 45%; min-width: 0; }
        :global(.srf-pair-tight) { flex-wrap: nowrap; align-items: flex-start; }
        :global(.srf-pair-qty) { flex: 0 0 auto; max-width: 30%; }
        :global(.srf-pair-size) { flex: 1 1 auto; }
        :global(.srf-pair-size .srf-size-row) { justify-content: flex-start !important; }
      `}</style>
    </>
  );
}