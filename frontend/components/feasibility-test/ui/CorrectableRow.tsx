...
export function SelectionReference({ rowId, limitingChecked, structuralChecked }: SelectionReferenceProps) {
  ...
  if (selected.length === 0) return (
    <div style={{ background: 'var(--bg-deep)', border: '1px solid var(--border)', borderRadius: 14, padding: '16px 18px', marginBottom: 24 }}>
      <p style={{ fontFamily: 'var(--font-ui)', fontSize: 9, letterSpacing: '0.25em', textTransform: 'uppercase', color: 'var(--accent)', margin: '0 0 10px' }}>
        Relevant Selections
      </p>
      <p style={{ fontFamily: 'var(--font-body)', fontSize: 14, color: 'var(--text-muted)', opacity: 0.5, margin: 0, fontStyle: 'italic' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        {selected.map(id => (
          <div key={id} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <span style={{ fontFamily: 'var(--font-ui)', fontSize: 12, letterSpacing: '0.07em', textTransform: 'uppercase', color: 'var(--text)', lineHeight: 1.4 }}>
              {LABEL_MAP[id] ?? id}
            </span>
          </div>
        ))}
  ...
  return (
    <div style={{ marginBottom: 32 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
        <span style={{ fontFamily: 'var(--font-ui)', fontSize: 9, fontWeight: 600, letterSpacing: '0.22em', textTransform: 'uppercase', color: 'var(--text-muted)' }}>
          {label}
        </span>
        {required && (
          <span style={{ fontFamily: 'var(--font-ui)', fontSize: 8, letterSpacing: '0.15em', textTransform: 'uppercase', color: 'rgba(255,211,105,0.7)', border: '1px solid rgba(255,211,105,0.35)', borderRadius: 4, padding: '2px 7px' }}>
            Required
          </span>
        )}
      </div>
