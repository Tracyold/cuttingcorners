// comp/admin/mobile/panels/users/panels/AdminUserInvoicesPanel.tsx
// Imports useAdminUserDetail for invoices data.
...
export default function AdminUserInvoicesPanel({ open, id, session, onClose, onBack, onDashboard }: Props) {
  ...
  const inputStyle: React.CSSProperties = { width: '100%', background: 'var(--bg-card)', border: '1px solid var(--bdr2-mob)', color: 'var(--text-mob)', fontFamily: 'var(--font-mono-mob)', fontSize: '1.0rem', padding: '12px 16px', outline: 'none', appearance: 'none', WebkitAppearance: 'none', transition: 'all 200ms ease', borderRadius: 8 };
  ...
  return (
    <div ref={elementRef} className={`slide-panel${open ? ' open' : ''}`}>
      ...
      <div style={{ padding: '20px 24px', borderBottom: '1px solid var(--bdr2-mob)', display: 'flex', flexDirection: 'column', gap: 16, background: 'var(--bg-deep)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <div style={{ flex: 1 }}>
            ...
            <input type="date" value={dateFrom} onChange={e => setDateFrom(e.target.value)} style={inputStyle} onFocus={e => { e.currentTarget.style.borderColor = 'var(--gold)'; }} onBlur={e => { e.currentTarget.style.borderColor = 'var(--bdr2-mob)'; }} />
          </div>
          <div style={{ flex: 1 }}>
            ...
            <input type="date" value={dateTo} onChange={e => setDateTo(e.target.value)} style={inputStyle} onFocus={e => { e.currentTarget.style.borderColor = 'var(--gold)'; }} onBlur={e => { e.currentTarget.style.borderColor = 'var(--bdr2-mob)'; }} />
          </div>
          <button onClick={() => { setDateFrom(''); setDateTo(''); }} style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid var(--bdr2-mob)', color: 'var(--text-mob)', fontSize: '1.0rem', width: 44, height: 44, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', flexShrink: 0, borderRadius: 8, marginTop: 20 }}>✕</button>
        </div>
        <span style={{ fontFamily: 'var(--font-mono-mob)', fontSize: '0.8125rem', letterSpacing: '0.18em', textTransform: 'uppercase', color: 'var(--text-mob-muted)' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflowY: 'auto' }}>
        {filtered.length === 0
          ? <div className="inv-empty"><div className="inv-empty-title">No invoices</div><div className="inv-empty-sub">Paid invoices will appear here</div></div>
          : <div className="inv-grid">
              {filtered.map((inv, idx) => {
                const item  = inv.line_items?.[0];
                const emoji = ['💎', '🟢', '🔴', '💛', '🔵'][idx % 5];
                return (
                  <div key={inv.invoice_id} className="inv-thumb">
