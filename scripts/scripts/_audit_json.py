...
FIXABILITY: dict[str, tuple[bool, str]] = {
    ...
    # audit_habits categories
    "ANY_TYPE":            (False, "Replace `any` with a specific type or `unknown`"),
    "MISSING_RETURN_TYPE": (False, "Add a return type annotation to the function"),
    "NON_NULL_ASSERTION":  (False, "Replace ! with proper null checks"),
    "UNSAFE_CAST":         (False, "Replace `as Type` with a type guard function"),
    "AWAIT_IN_LOOP":       (False, "Refactor to Promise.all() outside the loop"),
    "MAGIC_NUMBER":        (False, "Extract to a named constant"),
    "NO_PAGINATION":       (False, "Add .limit() or .range() to the query"),
    "DEEP_NESTING":        (False, "Extract inner blocks to named functions"),
    "EMPTY_CATCH":         (False, "Log or rethrow the error in the catch block"),
    "UNHANDLED_PROMISE":   (False, "Add .catch() or wrap in try/catch"),
    "EFFECT_NO_DEPS":      (False, "Add dependency array [] or list actual dependencies"),
    "INLINE_FUNCTION":     (False, "Extract to useCallback or a named handler"),
    "MISSING_KEY":         (False, "Add a stable key prop to each mapped element"),
    "FN_TOO_LONG":         (False, "Break into smaller focused functions"),
    "FILE_TOO_LONG":       (False, "Split into smaller single-responsibility modules"),
    "DUPLICATE_STRING":    (False, "Extract to a named constant"),
    "CONSOLE_LEFT_IN":     (True,  "Remove the console statement"),
    "COMMENTED_CODE":      (False, "Delete it — git history preserves it"),
    "TODO_IN_PROD":        (False, "Address the TODO or create a tracked issue"),
}
