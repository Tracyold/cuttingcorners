...
function collectDeadParams(node) {
  if (FUNC_KINDS.has(node.kind) && node.body) {
    ...
    (node.parameters || []).forEach(param => {
      // Skip 'this' pseudo-param — TypeScript type annotation, not a real parameter
      if (param.name.kind === ts.SyntaxKind.Identifier && param.name.text === 'this') return;
      // Skip rest params (...args) — variadic, consumed implicitly
      if (param.dotDotDotToken) return;
      // Skip destructured params ({ a, b }: Type) — binding patterns need deeper analysis
      if (param.name.kind !== ts.SyntaxKind.Identifier) return;

      const name = param.name.text;

      // Strip comments from body before checking usage
      // so a param only mentioned in a comment is still flagged as dead
      const bodyNoComments = bodyText
        .replace(/\/\/[^\n]*/g, '')           // single-line comments
        .replace(/\/\*[\s\S]*?\*\//g, '');    // multi-line comments

      const re = new RegExp('\\b' + name.replace(/[.*+?^${}()|[\]\\]/g, '\\$&') + '\\b');
      if (!re.test(bodyNoComments)) {
...
def _python_dead_params(file_path: Path) -> list[dict]:
    ...
    for node in _ast.walk(tree):
        ...
        for param_name, _, lineno in params:
            # Strip comments from body before checking — param only in a comment is still dead
            body_no_comments = re.sub(r'#[^\n]*', '', body_text)
            pattern = r'\b' + re.escape(param_name) + r'\b'
            if not re.search(pattern, body_no_comments):
                issues.append({
                    "name": param_name,
                    "fnName": fn_name,
                    "line": lineno,
                ...
                for m in re.finditer(
                    ...
                    for param in params_str.split(','):
                        ...
                        if not name or not re.match(r'^[a-zA-Z_]\w*$', name):
                            continue
                        # Strip comments from body before checking — param only in a comment is dead
                        body_no_comments = re.sub(r'//[^\n]*', '', body)
                        body_no_comments = re.sub(r'/\*[\s\S]*?\*/', '', body_no_comments)
                        if not re.search(r'\b' + re.escape(name) + r'\b', body_no_comments):
                            lineno = text[:m.start()].count('\n') + 1
